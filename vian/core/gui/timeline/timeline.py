...
class TimelineContainer(EDockWidget):
    def __init__(self, main_window):
        ...
        self.a_use_multi_annotation = self.menu_options.addAction("\tUse Multi-Annotation Mode")
...
class Timeline(QtWidgets.QWidget, IProjectChangeNotify, ITimeStepDepending):
    ...
    def add_sub_segmentation(self, target, sub:TimelineSubSegmentation, cat = "Global"):
        ...
        if cat not in self.item_sub_segmentations[target.get_id()]:
            ...
            group.show()
            self.item_sub_segmentations[target.get_id()][cat] = dict(group=[group, [], self.bar_height_min], entries = [])

        group = self.item_sub_segmentations[target.get_id()][cat]['group']
        ctrl = TimelineSubSegmentationControl(self.frame_Controls, self, target, sub=sub)
        group[0].add_child(ctrl)
        ctrl.show()
    ...
    def mouseMoveEvent(self, QMouseEvent):
        if QMouseEvent.buttons() & Qt.MouseButton.RightButton or self.shift_pressed:
    ...
    def create_segment(self, lst = None, mode=SegmentCreationMode.BACKWARD):
        ...
        if self.selected is not None and self.selected.get_type() == SEGMENTATION:
            ...
            self.main_window.show_info_popup(self.frame_Bars, "Please select a Segmentation before creating new Segments. ", Qt.Corner.TopLeftCorner)
