...
@vian_analysis
class AudioTempoAnalysis(IAnalysisJob):
    def __init__(self, resolution=30):
        super(AudioTempoAnalysis, self).__init__("Audio Tempo", [MOVIE_DESCRIPTOR, SEGMENT],
                                                 dataset_name="AudioTempo",
                                                 dataset_shape=(1,),
    ...
    def prepare(self, project: VIANProject, targets: List[IProjectContainer], fps, class_objs=None):
        ...
        targets, args = super(AudioTempoAnalysis, self).prepare(project, targets, fps, class_objs)
    ...
    def process(self, argst, sign_progress):
        ...
        for i in range(int(audio.shape[0] / window)):
            ...
            onset_env = librosa.onset.onset_strength(audio[w0:w1], sr=window)
            tempo = librosa.beat.tempo(audio[w0:w1], onset_envelope=onset_env)
    ...
    def modify_project(self, project: VIANProject, result: IAnalysisJobAnalysis, main_window=None):
        ...
        super(AudioTempoAnalysis, self).modify_project(project, result, main_window)
