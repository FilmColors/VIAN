...
# @vian_analysis
class AudioSegmentation(IAnalysisJob):
    """
    ...
    def prepare(self, project: VIANProject, targets: List[Segmentation], fps, class_objs=None):
        ...
        """
        super(AudioSegmentation, self).prepare(project, targets, fps, class_objs)
        args = dict(audio_path = os.path.join(project.data_dir, "audio.mp3"), movie_descriptor = project.movie_descriptor)
        return args
    ...
    def process(self, args, sign_progress):
        ...
        """
        args, sign_progress = super(AudioSegmentation, self).process(args, sign_progress)
        # Signal the Progress
        sign_progress(0.0)
    ...
    def get_parameter_widget(self):
        ...
        """
        return AudioSegmentationParameterWidget()


class AudioSegmentationParameterWidget(ParameterWidget):
    """
    ...
    def __init__(self):
        super(AudioSegmentationParameterWidget, self).__init__()
        # Put UI Here
