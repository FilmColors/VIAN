...
from PyQt5.QtGui import QColor, QImage, QPixmap, QWheelEvent, QKeyEvent, QMouseEvent, QPen, QFont, QBrush
...
class FeaturePlot(QGraphicsView):
    def __init__(self, parent, project: ElanExtensionProject, title = ""):
        ...
        self.fitInView(QRectF(300, 300, 500, 500), Qt.KeepAspectRatio)
    ...
    def on_add_feature(self, word):
        ...
        if len(rects) > 0:
            ...
            label = self.scene().addText(word.name, font)
            label.setPos(-100, y)
            label.setDefaultTextColor(QColor(255,255,255))
            self.features.append([word, rects, label])
    ...
    def create_timeline(self):
        ...
        for s in main_segm.segments:
            ...
            self.segments.append(self.scene().addRect(s.get_start() * self.magnification, 1200,
                                                      (s.get_end() - s.get_start()) * self.magnification, 200,
                                                      QColor(0, 0, 0)))
            self.segments.append(self.scene().addLine(s.get_start() * self.magnification,
